describe('Android Elements Test', () => {
    ...
    it.only('Home work', async () => {
        const viewsBTN = await $('~Views');
        viewsBTN.click();
        const autoCompleteBTN = await $('~Auto Complete');
        autoCompleteBTN.click();
        const screenTopBtn = await $('~1. Screen Top');
        screenTopBtn.click();
        const inputField = await $('//*[@resource-id="io.appium.android.apis:id/edit"]');
        await inputField.addValue('Israel');
        await expect(inputField).toHaveText('Israel');
